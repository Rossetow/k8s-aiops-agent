...
public class RetryHelper {
	...
	public static <T> T executeWithRetry(Callable<T> operation, String operationName) throws Exception {
		...
		while (attempt < MAX_RETRIES) {
			...
			try {
				...
				if (is429Error(e)) {
					...
					if (waitTime != null) {
						log.warn("Rate limit exceeded for {}, API suggests waiting {} seconds",
							operationName, waitTime.getSeconds());
						currentBackoff = waitTime;
					} else {
						log.warn("Rate limit exceeded for {} (attempt {}/{}), using exponential backoff: {} seconds",
					...
					if (attempt < MAX_RETRIES) {
						try {
							log.info("Waiting {} seconds before retry...", currentBackoff.getSeconds());
		...
		throw new RuntimeException(String.format("Max retries exceeded after %d attempts for %s", attempt, operationName), lastException);
	}
			...
			try {
				...
				if (shouldRetry) {
					if (is429Error(e)) {
						...
						log.warn("Transient Gemini API error for {} (attempt {}/{}): {}",
	...
	private static Duration extractRetryDelay(Exception e) {
		...
		if (matcher.find()) {
			try {
				...
				log.debug("Extracted retry delay from API: {} seconds", roundedSeconds);
				return Duration.ofSeconds(roundedSeconds);
			} catch (NumberFormatException nfe) {
				log.warn("Failed to parse retry delay: {}", matcher.group(1));
			}
		}
