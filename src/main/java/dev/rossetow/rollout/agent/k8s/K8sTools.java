...
/**
 * Kubernetes tools for Spring AI
 * All 8 tools are preserved and converted to Spring Component methods
 */
@Component
public class K8sTools {
    ...
    public K8sTools(KubernetesClient k8sClient) {
    ...
    public Map<String, Object> getLogs(String namespace, String podName, String containerName, Boolean previous, Integer tailLines) {
        ...
        try {
            ...
            // Log container analysis start
            log.info(MessageFormat.format("📦 Container Analysis - Pod: {0}/{1}", namespace, podName));
            if (containers != null && !containers.isEmpty()) {
                String containerNames = containers.stream()
                    .map(Container::getName)
                    .collect(Collectors.joining(", "));
                log.info(MessageFormat.format("📋 Available containers in pod: [{0}]", containerNames));
            }
            ...
            if (containerName == null || containerName.isEmpty()) {
                if (containers != null && containers.size() == 1) {
                    ...
                    selectionReason = "single container in pod";
                    log.info(MessageFormat.format("✅ Selected container: ''{0}'' (reason: {1})", targetContainer, selectionReason));
                    ...
                    List<String> sidecars = containers.stream()
                    ...
                    selectionReason = "first non-sidecar container";
                    if (!sidecars.isEmpty()) {
                        log.info(MessageFormat.format("❌ Skipped sidecar containers: [{0}]", String.join(", ", sidecars)));
                    }
                    log.info(MessageFormat.format("✅ Selected container: ''{0}'' (reason: {1})", targetContainer, selectionReason));
                } else {
                    targetContainer = null;
                    selectionReason = "no containers found";
                    log.warn("⚠️ No containers found in pod");
                }
            } else {
                ...
                if (!containerExists) {
                    ...
                    if (containers != null && containers.size() > 1) {
                        log.warn(MessageFormat.format("⚠️ Requested container ''{0}'' not found. Available: [{1}]. Auto-detecting...",
                            requestedContainer,
                    ...
                    if (containers != null && containers.size() == 1) {
                        targetContainer = containers.get(0).getName();
                        selectionReason = "fallback to single available container";
                        log.info(MessageFormat.format("✅ Selected container: ''{0}'' (reason: {1})", targetContainer, selectionReason));
                    } else if (containers != null && containers.size() > 1) {
                        List<String> sidecars = containers.stream()
                            .filter(c -> c.getName().contains("proxy") ||
                        ...
                        selectionReason = "fallback to first non-sidecar";
                        if (!sidecars.isEmpty()) {
                            log.info(MessageFormat.format("❌ Skipped sidecar containers: [{0}]", String.join(", ", sidecars)));
                        }
                        log.info(MessageFormat.format("✅ Selected container: ''{0}'' (reason: {1})", targetContainer, selectionReason));
                    ...
                    selectionReason = "explicitly requested";
                    log.info(MessageFormat.format("✅ Selected container: ''{0}'' (reason: {1})", targetContainer, selectionReason));
                }
            }
     ...
     */
    @Tool(description = "inspect resources")
     ...
     */
    @Tool(description = "parse label selector")
     ...
     *
     * This method queries the Argo Rollout resource to get the correct selectors,
     * then uses those selectors combined with role labels to find the exact pods
     * controlled by this rollout.
     *
     * Functionality:
     * 1. Fetches the Argo Rollout by name
     * 2. Extracts the base selector from the Rollout spec
     * 3. Combines base selector with role=stable to find stable pods
     * 4. Combines base selector with role=canary to find canary pods
     * 5. Gets logs from the first stable pod (if exists)
     * 6. Gets logs from the first canary pod (if exists)
     ...
     */
    @Tool(description = "get canary diagnostic")
    public Map<String, Object> getCanaryDiagnostics(String namespace, String rolloutName, String containerName, Integer tailLines) {
        log.info("=== Executing Tool: getCanaryDiagnostics ===");

        if (namespace == null || namespace.isEmpty()) {
            return Map.of("error", "namespace is required");
        }

        if (rolloutName == null || rolloutName.isEmpty()) {
            return Map.of("error", "rolloutName is required");
        }

        int lines = (tailLines != null && tailLines > 0) ? tailLines : 200;
        log.info(MessageFormat.format("Getting canary diagnostics for rollout: {0}/{1}, container: {2}, lines: {3}",
                namespace, rolloutName, containerName, lines));

        try {
            Map<String, Object> result = new HashMap<>();
            result.put("namespace", namespace);
            result.put("rolloutName", rolloutName);

            // Fetch the Rollout to get its selectors
            Rollout rollout = k8sClient.resources(Rollout.class)
                .inNamespace(namespace)
                .withName(rolloutName)
                .get();

            if (rollout == null) {
                return Map.of("error", MessageFormat.format("Rollout not found: {0}/{1}", namespace, rolloutName));
            }

            // Extract base selector from Rollout spec
            Map<String, String> baseSelector = new HashMap<>();
            if (rollout.getSpec() != null &&
                rollout.getSpec().getSelector() != null &&
                rollout.getSpec().getSelector().getMatchLabels() != null) {
                baseSelector.putAll(rollout.getSpec().getSelector().getMatchLabels());
            }

            log.info(MessageFormat.format("Base selector from Rollout: {0}", baseSelector));

            // Create selectors for stable and canary by combining base selector with role
            Map<String, String> stableSelector = new HashMap<>(baseSelector);
            stableSelector.put("role", "stable");

            Map<String, String> canarySelector = new HashMap<>(baseSelector);
            canarySelector.put("role", "canary");

            log.info(MessageFormat.format("Stable selector: {0}", stableSelector));
            log.info(MessageFormat.format("Canary selector: {0}", canarySelector));
            ...
            // Log pod counts
            log.info(MessageFormat.format("🔵 Analyzing STABLE deployment - Found {0} pod(s)", stablePods.size()));
            if (!stablePods.isEmpty()) {
                String stablePodNames = stablePods.stream()
                    .map(p -> p.getMetadata().getName())
                    .limit(3)
                    .collect(Collectors.joining(", "));
                log.info(MessageFormat.format("   Stable pods (showing up to 3): [{0}]", stablePodNames));
            }

            log.info(MessageFormat.format("🟡 Analyzing CANARY deployment - Found {0} pod(s)", canaryPods.size()));
            if (!canaryPods.isEmpty()) {
                String canaryPodNames = canaryPods.stream()
                    .map(p -> p.getMetadata().getName())
                    .limit(3)
                    .collect(Collectors.joining(", "));
                log.info(MessageFormat.format("   Canary pods (showing up to 3): [{0}]", canaryPodNames));
            }
            ...
            CompletableFuture<Map<String, Object>> stableInfoFuture = CompletableFuture.supplyAsync(() -> {
                ...
                if (!stablePods.isEmpty()) {
                    ...
                    log.info(MessageFormat.format("🔵 Processing STABLE pod: {0}", stablePod.getMetadata().getName()));
                    ...
                    if (logsResult.containsKey("logs")) {
                        ...
                        String analyzedContainer = logsResult.get("container") != null ? logsResult.get("container").toString() : "unknown";
                        log.info(MessageFormat.format("📊 STABLE container analyzed: ''{0}''", analyzedContainer));
                    } else if (logsResult.containsKey("error")) {
                        stableInfo.put("logsError", logsResult.get("error"));
                        log.warn(MessageFormat.format("⚠️ Failed to get STABLE logs: {0}", logsResult.get("error")));
                    }
                } else {
                    stableInfo.put("error", "No stable pods found");
                    log.warn("⚠️ No STABLE pods found with selector");
                }
                return stableInfo;
            });
            ...
            CompletableFuture<Map<String, Object>> canaryInfoFuture = CompletableFuture.supplyAsync(() -> {
                ...
                if (!canaryPods.isEmpty()) {
                    ...
                    log.info(MessageFormat.format("🟡 Processing CANARY pod: {0}", canaryPod.getMetadata().getName()));
                    ...
                    if (logsResult.containsKey("logs")) {
                        ...
                        String analyzedContainer = logsResult.get("container") != null ? logsResult.get("container").toString() : "unknown";
                        log.info(MessageFormat.format("📊 CANARY container analyzed: ''{0}''", analyzedContainer));
                    } else if (logsResult.containsKey("error")) {
                        canaryInfo.put("logsError", logsResult.get("error"));
                        log.warn(MessageFormat.format("⚠️ Failed to get CANARY logs: {0}", logsResult.get("error")));
                    }
                } else {
                    canaryInfo.put("error", "No canary pods found");
                    log.warn("⚠️ No CANARY pods found with selector");
                }
                return canaryInfo;
            });
            ...
            log.info("✅ Successfully retrieved canary diagnostics (parallel execution completed)");
            log.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            return result;
